...
int main()
{
  ...
  for (i = MIN_DATE; lessThan(i, MAX_DATE); )
  {
    if (isPalindrome(i))
    {
      cout << "\nIs palindrome" << endl;
...
void displayDate ( const Date& date, ostream& ostr, DATE_STYLE dStyle )
{
  ...
  else
  {
    if (dStyle == MM_DD_YYYY)
    {
      ...
      (dtM < BASE_TEN) ? ostr << zero << dtM : ostr << dtM;

      ostr << "/";

      (dtD < BASE_TEN) ? ostr << zero << dtD : ostr << dtD;
...
Date incrementDate ( const Date& date )
{
  const unsigned dtM = month(date), dtD = day(date),
                 dtY = year(date), dtMLen = monthLength(date);
  Date newDate;
  ...
  else
  {
    ...
    {
      newDate = makeDate((dtM - 11),(dtMLen - 30), (dtY + 1));
    }
    //day goes back to 01 and month increases by 1
    else if (dtD == dtMLen)
    {
      newDate = makeDate(dtM + 1,(dtD - dtMLen) + 1, dtY);
    }
    else newDate = makeDate(dtM, dtD + 1, dtY);
  }
  return newDate;
}
...
bool wellFormed ( const Date& date )
{
  ...
  if (date >= MIN_DATE && date <= MAX_DATE) isWellFormed = true;
  else cout << ERR_MSG;
...
unsigned monthLength ( const Date& date )
{
  ...
  if (wellFormed(date))
  {
    ...
    if (dateMonthNum == FEB)
    {
      (isLeapYear(date)) ? daysInMonth = 29 : daysInMonth = 28;
    }
    //Months with 30 days
